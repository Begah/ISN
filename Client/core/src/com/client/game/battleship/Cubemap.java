...
public class Cubemap implements Disposable {
	...
	public Cubemap(FileHandle positiveX, FileHandle negativeX, FileHandle positiveY, FileHandle negativeY,
		...
		this(new Pixmap(positiveX), new Pixmap(negativeX), new Pixmap(positiveY), new Pixmap(negativeY),
				new Pixmap(positiveZ), new Pixmap(negativeZ));
	}
	...
	public Cubemap(Pixmap cubemap) {
		...
		for (int i = 0; i < 6; i++)
			data[i] = new Pixmap(w / 4, h / 3, Format.RGB888);
		for (int x = 0; x < w; x++)
			for (int y = 0; y < h; y++) {
				// -X
	...
	public void render(Camera camera, Vector2 plane) {
	...
	public static Mesh createQuad() {
		...
		mesh.setVertices(new float[] { -1f, -1f, 0, 1, 1, 1, 1, 0, 1, 1f, -1f, 0, 1, 1, 1, 1, 1, 1, 1f, 1f, 0, 1, 1, 1,
				1, 1, 0, -1f, 1f, 0, 1, 1, 1, 1, 0, 0 });
